...
- 📝 **중복 알림 방지**: 하루 1회 알림 발송 여부를 기록해 같은 날 중복 발송 방지
- 📈 **환율 이력 누적**: 실행할 때마다 조회한 환율 데이터를 JSON 파일에 계속 쌓아, 추후 시간대별 변동 추이 분석 가능
...
├── last_rate.json             # 하루 중복 알림 방지용 상태 (마지막 발송 날짜만 기록)
├── rate_history.json          # 실행할 때마다 누적되는 환율 이력 데이터
└── .github/
    └── workflows/
